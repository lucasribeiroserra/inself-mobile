-- InSelf: schema inicial + RLS + seed mínimo
...
-- ========== JORNADA (check-in → reflexão → check-out) ==========
...
-- Índices para consultas por user_id
...
ALTER TABLE public.reflection_content ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.reflection_steps ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.badges ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.challenges ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.challenge_days ENABLE ROW LEVEL SECURITY;

-- Conteúdo: leitura pública para usuários autenticados
CREATE POLICY "Conteúdo legível por autenticados" ON public.emotions FOR SELECT TO authenticated USING (true);
CREATE POLICY "Conteúdo legível por autenticados" ON public.categories FOR SELECT TO authenticated USING (true);
CREATE POLICY "Conteúdo legível por autenticados" ON public.virtues FOR SELECT TO authenticated USING (true);
CREATE POLICY "Conteúdo legível por autenticados" ON public.reflection_content FOR SELECT TO authenticated USING (true);
CREATE POLICY "Conteúdo legível por autenticados" ON public.reflection_steps FOR SELECT TO authenticated USING (true);
CREATE POLICY "Conteúdo legível por autenticados" ON public.badges FOR SELECT TO authenticated USING (true);
CREATE POLICY "Conteúdo legível por autenticados" ON public.challenges FOR SELECT TO authenticated USING (true);
CREATE POLICY "Conteúdo legível por autenticados" ON public.challenge_days FOR SELECT TO authenticated USING (true);

-- Tabelas por usuário: só o próprio user_id
...
ALTER TABLE public.reflection_session_steps ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.emotional_checkouts ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.user_reflections ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.user_reflection_favorites ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.user_badges ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.virtue_progress ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.user_streaks ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.user_challenges ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.user_challenge_days ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Perfil próprio" ON public.user_profiles FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Settings próprio" ON public.user_settings FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Checkins próprio" ON public.emotional_checkins FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Sessões próprio" ON public.reflection_sessions FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Steps próprio" ON public.reflection_session_steps FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.reflection_sessions rs WHERE rs.id = reflection_session_id AND rs.user_id = auth.uid()));
CREATE POLICY "Checkouts próprio" ON public.emotional_checkouts FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Reflexões próprio" ON public.user_reflections FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Favoritos próprio" ON public.user_reflection_favorites FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Badges próprio" ON public.user_badges FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Virtude progress próprio" ON public.virtue_progress FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Streak próprio" ON public.user_streaks FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Desafios próprio" ON public.user_challenges FOR ALL TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Desafio dias próprio" ON public.user_challenge_days FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.user_challenges uc WHERE uc.id = user_challenge_id AND uc.user_id = auth.uid()));

-- ========== SEED MÍNIMO (emotions, categories, virtues, 1 reflection + steps, badges) ==========
...
-- Uma reflexão de exemplo (ansiedade / serenidade)
...
-- Mais reflexões do app atual para ter conteúdo
INSERT INTO public.reflection_content (category_id, virtue_id, quote, author, message)
...
-- Trigger: ao criar usuário no Auth, criar perfil, settings e streak
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS trigger AS $$
BEGIN
  INSERT INTO public.user_profiles (user_id, locale) VALUES (NEW.id, 'pt-BR');
